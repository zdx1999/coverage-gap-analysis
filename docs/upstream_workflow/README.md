...
- `06_validity_robustness/`: robustness scripts for uneven Weibo temporal coverage, together with final coverage-gap validity checks.
- `docs/upstream_workflow/file_manifest.csv`: source-to-package manifest from the original curated submission code package.
...
The current manuscript figures and coverage-gap results are reproduced by the updated scripts under `04_coverage_gap_analysis/`, `05_figure_generation/` and `06_validity_robustness/`; those scripts supersede older figure-generation files from the April 2026 submission package.
